...
import com.example.demo.twilio.SmsSender;
...
public class RegistrationService {
	...
	@Autowired
	private ConformationTokenService confirmationTokenService;
	...
	@Autowired
	public RegistrationService(@Qualifier("twilio") TwilioSmsSender twilioSmsSender) {
		super();
	...
	public String register(RegistrationRequest request) {
		...
		String token = appUserService.signUpUser(new AppUser(request.getFirstName(), request.getLastName(),
				request.getEmail(), request.getPassword(), request.getPhoneNumber(), AppUserRole.USER));
		String link = "http://localhost:8081/api/v1/registration/confirm?token=" + token;
		String smsMessage = "Dear customer please verify your Vector account in 10 Minutes: " + link + "Or else use this token : "+token;
		emailSender.send(request.getEmail(), buildEmail(request.getFirstName(), link));
		twilioSmsSender.sendSms(request.getPhoneNumber(), smsMessage);
		return token;
	}
